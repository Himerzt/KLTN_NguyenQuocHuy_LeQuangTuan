...
MODEL_WEIGHTS = r"kp_only_missing.weights.h5"
    ...
    async def processor():
        ...
        try:
            while True:
                ...
                # Khởi tạo predictions tạm thời và gửi về frontend
                predictions = {}
                ...
                async def process_person(det):
                    ...
                    # Normalize keypoints trong phạm vi bbox
                    kp_norm = person_kps.copy()
                    kp_norm[:, 0] = np.clip(kp_norm[:, 0] / w, 0.0, 1.0)
                    kp_norm[:, 1] = np.clip(kp_norm[:, 1] / h, 0.0, 1.0)

                    # copy phần (x_norm, y_norm) để gửi sang frontend
                    kp_norm_xy = kp_norm[:, :2].copy().flatten()  # shape: (25*2,) = 50
